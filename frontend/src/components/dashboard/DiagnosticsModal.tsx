...
export default function DiagnosticsModal({ isOpen, onClose }: DiagnosticsModalProps) {
  ...
  const runAllDiagnostics = async () => {
    ...
    try {
      ...
      // ─── CATEGORY 5: WEBSOCKET HEALTH (Simulate Disconnect/Reconnect Loop) ───
      ...
      // Image URLs verify (Fetch Clearbit logo as live verification)
      ...
      // ─── CATEGORY 7: PERFORMANCE METRICS (Fetch page render speeds) ───
      const perfPages = [
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      {/* Modal Card wrapper */}
      <div className="bg-white border-4 border-black rounded-3xl shadow-[8px_8px_0px_#000000] w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-fadeIn">
        {/* Modal Header */}
        <div className="p-6 border-b-4 border-black flex justify-between items-center bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black uppercase tracking-tight">Developer Diagnostics Console</h2>
              <p className="text-xs text-gray-500 font-bold mt-0.5">Automated deep-system audits · Non-production diagnostics suite</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="p-2 border-3 border-black bg-white hover:bg-gray-100 rounded-xl cursor-pointer transition-colors"
          >
        ...
        <div className="flex border-b-3 border-black bg-[#F8FAFC] overflow-x-auto select-none">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4.5 py-3.5 font-black text-[10px] uppercase tracking-wider border-r-3 border-black cursor-pointer transition-colors ${
                activeTab === tab.key
        ...
        <div className="flex-grow overflow-y-auto p-6 bg-[#FAFBFF]">
          
          {/* TAB 1: SUMMARY TAB */}
          {activeTab === "summary" && (
            <div className="space-y-6">
              {/* Overall diagnostics report header */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Score panel */}
                <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000] flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] font-black uppercase text-gray-500 mb-2">Overall Health</span>
                  <div className={`text-5xl font-black ${getScoreColor(scores.overall)}`}>
                    {scores.overall}%
                  </div>
                  <span className="text-xs font-bold text-gray-400 mt-2 uppercase tracking-wide">Stockox Engine Status</span>
                </div>

                {/* Score breakdown metrics list */}
                <div className="md:col-span-2 border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000] space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={item.name} className="flex justify-between items-center text-xs font-bold bg-[#F8FAFC] p-2 border border-gray-200 rounded-lg">
              ...
              <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000]">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={srv.name} className="flex flex-col justify-between border-2 border-black/8 rounded-xl p-3 bg-[#F8FAFC] shadow-[2px_2px_0px_#00000008]">
                      <div className="flex items-center gap-2 mb-3">
                        {srv.icon}
                        <span className="text-[10px] font-black uppercase tracking-tight text-gray-700 truncate">{srv.name}</span>
                      </div>
          ...
          {activeTab === "services" && (
            <div className="space-y-6">
              {/* Auth details & Database counts */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Authentication Health Details */}
                <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000]">
                  <h3 className="text-xs font-black uppercase tracking-wider text-[#0F172A] border-b-2 border-gray-100 pb-2 mb-3 flex items-center gap-1.5">
                  ...
                  {authStatus && (
                    <div className="space-y-2.5 text-xs font-bold">
                      ...
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        ...
                        <span className="font-mono text-[10px] text-gray-600 truncate max-w-[180px]" title={authStatus.userId}>{authStatus.userId || "—"}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">{authStatus.latency}</span>
                      </div>
                    </div>
                  )}
                </div>

                {/* Database counts & GORM query pings */}
                <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000]">
                  <h3 className="text-xs font-black uppercase tracking-wider text-[#0F172A] border-b-2 border-gray-100 pb-2 mb-3 flex items-center gap-1.5">
                  ...
                  {dbStatus && (
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-xs font-bold border-b border-gray-100 pb-1.5">
                        ...
                        <span className="font-mono">{dbStatus.latency}</span>
                      </div>
                  ...
                  {cacheStatus && (
                    <div className="space-y-2.5 text-xs font-bold">
                      ...
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        ...
                        <span className="font-mono">{cacheStatus.keys}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        <span className="text-gray-400">Memory Allocation</span>
                        <span className="font-mono">{cacheStatus.memory}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        <span className="text-gray-400">Hits / Misses</span>
                        <span className="font-mono">{cacheStatus.hits} / {cacheStatus.misses}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        <span className="text-gray-400">Hit Rate Ratio</span>
                        <span className="font-black text-emerald-600">{cacheStatus.hitRate}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        <span className="text-gray-400">Write Latency</span>
                        <span className="font-mono">{cacheStatus.writeLatency || "—"}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                        <span className="text-gray-400">Read Latency</span>
                        <span className="font-mono">{cacheStatus.readLatency || "—"}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 pb-1.5">
                  ...
                  {marketStatus && (
                    <div className="space-y-3">
                      {marketStatus.map((p: any) => (
                        <div key={p.provider} className="flex items-center justify-between p-2 bg-[#F8FAFC] border border-gray-200 rounded-xl">
                          <div>
              ...
              {/* WebSocket and Search & Storage status details */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2.5 text-xs font-bold">
                      {searchStatus.map((t: any) => (
                        <div key={t.symbol} className="flex justify-between items-center border-b border-gray-100 pb-1.5">
                          <span className="font-mono text-[#0F172A] border-2 border-black rounded px-1 text-[10px]">{t.symbol}</span>
                          <span className="text-[10px] font-bold text-gray-400">Time: {t.latency} · Records: {t.count}</span>
                ...
                {/* WebSockets & Storage */}
                <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000] space-y-4">
                  {/* WebSockets */}
                  <div>
                    ...
                    {wsStatus && (
                      <div className="space-y-1.5 text-xs font-bold">
                        <div className="flex justify-between">
                          <span className="text-gray-400">Broadcasting Server Connection</span>
                          <span>{wsStatus.connected ? "CONNECTED" : "OFFLINE"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">Socket Latency</span>
                          <span className="font-mono">{wsStatus.latency}</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <h3 className="text-xs font-black uppercase tracking-wider text-[#0F172A] border-b border-gray-100 pb-1.5 mb-2 flex items-center gap-1.5">
                      <HardDrive className="w-4 h-4 text-[#2563EB]" /> File Storage (Avatar S3 Bucket)
                    </h3>
                    {storageStatus && (
                      <div className="space-y-1.5 text-xs font-bold">
                        <div className="flex justify-between">
                          <span className="text-gray-400">Storage Provider</span>
                          <span>{storageStatus.provider}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">CRUD Lifecycle Pings</span>
                          <span className="text-[10px] text-gray-500 font-bold">Upload: {storageStatus.uploadLatency} · Read: {storageStatus.readLatency}</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {envVars.map((ev: any) => (
                    <div key={ev.name} className="flex justify-between items-center p-2 bg-[#F8FAFC] border border-black/10 rounded-lg">
                      <span className="font-mono text-[9px] font-bold text-gray-600 truncate max-w-[180px]" title={ev.name}>{ev.name}</span>
          ...
          {/* TAB 3: API TAB */}
          {activeTab === "api" && (
            <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000] overflow-hidden">
                    ...
                    {apiTests.map((t, idx) => (
                      <tr key={idx} className="border-b border-gray-100 hover:bg-[#F8FAFC] text-xs font-bold">
                        <td className="px-3 py-2.5">
                          ...
                          <span className="font-mono text-[10px] text-gray-700">{t.endpoint}</span>
                        </td>
                        <td className="px-3 py-2.5">
                        ...
                        <td className="px-3 py-2.5 font-mono text-[10px] font-black">{t.responseTime}</td>
                        <td className="px-3 py-2.5 font-mono text-[10px] text-gray-400">{t.payloadSize}</td>
          ...
          {activeTab === "agents" && (
            <div className="space-y-6">
              <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000]">
                <div className="flex justify-between items-center mb-4 border-b-2 border-gray-100 pb-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#2563EB] text-white border-2 border-black rounded-lg font-black text-[9px] uppercase shadow-[2px_2px_0px_#000] hover:bg-[#1D4ED8] disabled:opacity-50 cursor-pointer flex items-center gap-1"
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${testingBand ? "animate-spin" : ""}`} />
                    Run Integration check (spawns test room)
                  </button>
                </div>
              ...
              {/* Debate pipeline details */}
              <div className="border-4 border-black rounded-2xl bg-white p-5 shadow-[4px_4px_0px_#000] space-y-3 font-mono text-[10px] text-gray-600">
                <h3 className="text-xs font-black uppercase tracking-wider text-[#0F172A] border-b-2 border-gray-100 pb-2 font-sans">Debate Integration Test Verification (Target: NVDA)</h3>
                ...
                {!testingBand && bandTestResult && (
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center py-1 border-b border-gray-100">
                      <span>✓ Spawning agents in test room ({bandTestResult.room_id || "diagnostics-test-nvda"}):</span>
                    ...
                    <div className="flex justify-between items-center py-1 border-b border-gray-100">
                      <span>✓ Deleting temporary test room & records:</span>
                      <span className="font-black text-emerald-600">{bandTestResult.success ? "PASS" : "FAIL"}</span>
                    </div>
                    ...
                    {bandTestResult.success && (
                      <div className="mt-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl font-sans text-xs font-bold text-emerald-800">
                        <p className="text-emerald-700">✓ Parallel execution test passed in {bandTestResult.latency_ms}ms!</p>
                        <p className="mt-1">Resolved Consensus Decision: <span className="font-black">{bandTestResult.committee_decision}</span> (Confidence: {bandTestResult.confidence}%)</p>
                      </div>
                    )}
                    ...
                    {!bandTestResult.success && (
                      <div className="mt-4 p-3 bg-rose-50 border border-rose-200 rounded-xl font-sans text-xs font-bold text-rose-800">
                        ...
                        <p className="mt-1 font-mono text-[10px] text-rose-600 whitespace-pre-wrap">{bandTestResult.error}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {perfMetrics.map((p) => (
                  <div key={p.name} className="flex justify-between items-center border border-gray-200 rounded-xl p-3 bg-[#F8FAFC]">
                    <div>
                      ...
                      <span className="text-[10px] text-gray-400 font-bold">Route Resolution Time: <span className="text-black font-black">{p.latency}</span></span>
                    </div>
                ...
                {recentErrors.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {recentErrors.map((err, idx) => (
                      <div key={idx} className="p-3 bg-rose-50/50 border-2 border-rose-200 rounded-xl space-y-1.5 font-mono text-[9px]">
        ...
        <div className="p-6 border-t-4 border-black bg-white flex justify-between items-center">
          <div className="text-[10px] text-gray-400 font-black uppercase tracking-wider flex items-center gap-1.5">
